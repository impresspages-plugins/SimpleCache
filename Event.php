...
class Event
{
    ...
    public static function ipInitFinished_1()
    {
        ...
        $cache = $model->getPageCache((empty($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
    ...
    public static function ipBeforeResponseSent_1000($info)
    {
        ...
        $model = Model::instance();
        $model->setPageCache($page->getId(), (empty($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], $html);
    }
